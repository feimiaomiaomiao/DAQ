...
/**********************************************************************************/
 /* ADCCLK = PCLK2/6   72/6 = 12MHZ
  打开ADC时钟，ADC时钟来自APB2总线
  ADCCLK最大14MHZ  来自参考手册11.1  155页
  */
  ...
  */
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1 | RCC_APB2Periph_ADC2 | RCC_APB2Periph_GPIOA, ENABLE);
   /* Enable DMA1 clock 使能DMA1时钟*/
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);
/**********************************************************************************/
  /* Configure PA.01, PA.02 and PA.03 (ADC Channel12,  as analog inputs 
...
/* DMA1 channel1 configuration  DMA1通道1配置，能且只能通道1参考手册10.3.7  147页 
...
//  DMA_InitStructure.DMA_PeripheralBaseAddr = ADC1_DR_Address;//与下面一句是等价的
  ...
  DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;       //循环模式，循环模式打开就是说，传输完，会把传输计数器的值加载回去
  DMA_InitStructure.DMA_Priority = DMA_Priority_High;   //高优先级
  DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;          //外设到存储器之间的搬运 不是M到M（存储器到存储器）所以是失能
  DMA_Init(DMA1_Channel1, &DMA_InitStructure);
  
  /* Enable DMA1 channel1 使能DMA1*/
  DMA_Cmd(DMA1_Channel1, ENABLE);


/**********************************************************************************/

/* ADC1 configuration -ADC1配置-------------------------------------*/
  //独立模式，ADC1和ADC2各干各的
  ADC_InitStructure.ADC_Mode = ADC_Mode_Independent;
  
  ADC_InitStructure.ADC_ScanConvMode = ENABLE;//多通道扫描模式（ENABLE）  单通道（DISABLE）
  ADC_InitStructure.ADC_ContinuousConvMode = DISABLE;//连续转换（ENABLE） 单次转换（DISABLE）
...
/* Enable ADC1 使能ADC1*/
 ADC_Cmd(ADC1, ENABLE);
  /* Enable ADC1 reset calibration register ADC校准（固定步骤）参考手册11.4*/   
...
/* Start ADC1 Software Conversion 启动ADC1转换*/ 
...
{
  /*如果ADC_ContinuousConvMode是单次转换，那么每次转换时都要启动ADC1转换*/ 
  ADC_SoftwareStartConvCmd(ADC1, ENABLE);
